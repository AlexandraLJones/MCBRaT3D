...
!-------------------------------------------------------------------------------------------
  function newPhotonStream_LWemission(numberOfPhotons, atms_photons, voxel_weights, nx, ny, nz, randomNumbers, status) result(photons)
    ...
    real, dimension(0:nx*ny*nz), intent(in)         :: voxel_weights

    ! Local variables
    integer :: i, numberOfAtmsPhotons, startPhoton, ind, ii, ij, ik
    real    :: RN!, test
...
!PRINT *, "i=", i, " RN=", RN
            !determine the index of the emitting pixel
!            temp1=voxel_weights(:)-RN
!            temp2=ABS(temp1)
!            ind=MINLOC(temp2,1)
            ind = MINLOC(ABS(voxel_weights(:)-RN),1)
            if(voxel_weights(ind) .lt. RN) ind = ind + 1
            !now convert linear index into 3D components
            ik = ((ind-1)/(nx*ny)) + 1
            ij = (((ind-1)-((ik-1)*nx*ny))/nx) + 1
            ii = ind - ((ik-1)*nx*ny) - ((ij-1)*nx)
            ...
            photons%zPosition(i) = ((ik-1)*(1.0)/nz) + (RN/nz) ! The first term represents the fractional position of the layer bottom in the column, such that ik=1 corresponds to a position of 0. The second term respresents the position within the layer.
...
!---------------------------------------------------------------------------------------------
  subroutine emission_weighting(nx, ny, nz, numComponents, xPosition, yPosition, zPosition, lambda_u, totalPhotons, atmsPhotons, voxel_weights, atmsTemp, ssas, cumExt,  sfcTemp, emiss, totalFlux)
...
! Updated Fall 2012 to remove predetermination of number of photons emitted per column
     ...
     real, dimension(0:nx*ny*nz), intent(out)          :: voxel_weights
     ...
     do iz = 1, nz
       do iy = 1, ny
         do ix = 1, nx
           ...
           voxel_weights(((iz-1)*nx*ny)+((iy-1)*nx)+ix) = voxel_weights(((iz-1)*nx*ny)+((iy-1)*nx)+ix-1) + 4.0*Pi* atmsPlanckRad * totalAbsCoef*dz(iz)     ! [Wm^-2] 

         end do
       end do
     end do 
          if (voxel_weights(last) .gt. 0.0) then
               atmsPower = voxel_weights(last)*((xPosition(nx+1)-xPosition(1))*(yPosition(ny+1)-yPosition(1))*(1000**2))  ! [W] total power emitted by atmosphere. Factor of 1000^2 is to convert dx and dy from km to m
               voxel_weights(:)=voxel_weights(:)/voxel_weights(last)     ! normalized
               voxel_weights(last)=1.0     ! need this to be 1 for algorithm used to select emitting voxel
          end if
