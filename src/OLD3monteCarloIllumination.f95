...
!-------------------------------------------------------------------------------------------
  function newPhotonStream_LWemission(numberOfPhotons, atms_photons, voxel_weights, col_weights, level_weights, nx, ny, nz, randomNumbers, status) result(photons)
    ...
    real, dimension(nx,ny,nz), intent(in)         :: voxel_weights
    real, dimension(ny,nz), intent(in)         :: col_weights
    real, dimension(nz), intent(in)         :: level_weights
    ! Local variables
...
!            temp2=ABS(temp1)
!            ind=MINLOC(temp2,1)
            ik = MINLOC(ABS(level_weights(:)-RN),1)
            if(level_weights(ik) .lt. RN) ik = ik + 1
PRINT *, 'ik= ', ik, 'level_weights=', level_weights(ik)

            ij = MINLOC(ABS(col_weights(:,ik)-RN),1)
            if(col_weights(ij,ik) .lt. RN) ij = ij + 1
PRINT *, 'ij= ', ij, 'col_weights=', col_weights(ij,ik)

            ii = MINLOC(ABS(voxel_weights(:,ij,ik)-RN),1)
            if(voxel_weights(ii,ij,ik) .lt. RN) ii = ii + 1
PRINT *, 'ii= ', ii, 'voxel_weights=', voxel_weights(ii,ij,ik)
...
! Updated Fall 2012 to remove predetermination of number of photons emitted per column
     ...
     real, dimension(nx,ny,nz), intent(out)          :: voxel_weights
     real, dimension(ny,nz), intent(out)          :: col_weights
     real, dimension(nz), intent(out)          :: level_weights
     ...
     do iz = 1, nz
       do iy = 1, ny
         ...
         col_weights(iy,iz)= previous
          PRINT *, ix, iy, iz, 'voxel_weights= ', voxel_weights(ix,iy,iz), 'col_weights= ', col_weights(iy,iz)
       end do   ! j loop
       level_weights(iz)= previous
       PRINT *, ix, iy, iz, 'voxel_weights= ', voxel_weights(ix,iy,iz), 'col_weights= ', col_weights(iy,iz), 'level_weights= ', level_weights(iz)
     end do     ! k loop
          if (voxel_weights(nx,ny,nz) .gt. 0.0) then
               atmsPower = voxel_weights(nx,ny,nz)*((xPosition(nx+1)-xPosition(1))*(yPosition(ny+1)-yPosition(1))*(1000**2))  ! [W] total power emitted by atmosphere. Factor of 1000^2 is to convert dx and dy from km to m
               voxel_weights(:,:,:)=voxel_weights(:,:,:)/voxel_weights(nx,ny,nz)     ! normalized
               col_weights(:,:)=col_weights(:,:)/voxel_weights(nx,ny,nz)
               level_weights(:)=level_weights(:)/voxel_weights(nx,ny,nz)
