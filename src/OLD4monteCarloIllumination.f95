...
!if(i .eq. 1) PRINT *, 'ik= ', ik, 'level_weights=', level_weights(ik), 'level_weights-RN=', level_weights(:)-RN, 'ABS=', ABS(level_weights(:)-RN)

            ij = MINLOC(ABS(col_weights(:,ik)-RN),1)
            if(col_weights(ij,ik) .lt. RN) ij = ij + 1
!if(i .eq. 1)PRINT *, 'ij= ', ij, 'col_weights=', col_weights(ij,ik), 'col_weights-RN=', col_weights(:,:)-RN, 'ABS=', ABS(col_weights(:,:)-RN)

            ii = MINLOC(ABS(voxel_weights(:,ij,ik)-RN),1)
            if(voxel_weights(ii,ij,ik) .lt. RN) ii = ii + 1
!if(i .eq. 1)PRINT *, 'ii= ', ii, 'voxel_weights=', voxel_weights(ii,ij,ik),'voxel_weights-RN=', voxel_weights(:,:,:)-RN,'ABS=',ABS(voxel_weights(:,:,:)-RN)
PRINT *, RN, ii, ij, ik
