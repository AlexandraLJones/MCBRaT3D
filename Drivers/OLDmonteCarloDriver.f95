...
program monteCarloDriver
  ...
  integer, allocatable :: atmsColumn_photons(:,:)
  real, allocatable    :: vertical_weights(:,:,:), cumExt(:,:,:), temps(:,:,:)
...
!    if(phiFill(2) .eq. phiFill(1)) nPhi = 1
    ...
    mus = thetaFill(1)+(real((/0:(nMu-1)/))*thetaFill(3)) 
      mus = cos(Pi/180. * mus)
    phis= phiFill(1)  +(real((/0:(nPhi-1)/))*phiFill(3)) 
    end if
...
!PRINT *, 'solarFlux=', solarFlux
  if(LW_flag >= 0.0)then
     allocate (atmsColumn_photons(1:nX,1:nY), vertical_weights(1:nX,1:nY,1:nZ+1), temps(1:nX,1:nY,1:nZ), cumExt(1:nX,1:nY,1:nZ), ssa(1:nX,1:nY,1:nZ,1:numberOfComponents))
...
!print *, 'Driver: got info integrator'
     call emission_weighting(nX, nY, nZ, numberOfComponents, xPosition, yPosition, zPosition, lambda, numPhotonsPerBatch, atmsColumn_photons, vertical_weights, temps, ssa, cumExt, surfaceTemp, (1.-surfaceAlbedo), emittedFlux) 
     solarFlux=emittedFlux
!PRINT *, 'total atms photons=', sum(atmsColumn_photons)
!PRINT *, 'emittedFlux=', emittedFlux, ' solarFlux=', solarFlux
!PRINT *, 'Driver: calculated emission weighting'
     incomingPhotons = new_PhotonStream (numberOfPhotons=1, atmsColumn_photons=atmsColumn_photons, vertical_weights=vertical_weights, nX=nX, nY=nY, nZ=nZ, randomNumbers=randoms, status=status)  
...
!PRINT *, "entering batch loop"
  batches: do batch = thisProc*batchesPerProcessor + 1, thisProc*batchesPerProcessor + batchesPerProcessor
    ...
    !   stored in an "illumination" object. 
    if(LW_flag >= 0.0)then
       incomingPhotons = new_PhotonStream (numberOfPhotons=numPhotonsPerBatch, atmsColumn_photons=atmsColumn_photons, vertical_weights=vertical_weights, nX=nX, nY=nY, nZ=nZ, randomNumbers=randoms, status=status)  
...
!PRINT *, 'Driver: Reported results for batch', batch
  ...
  if (allocated(atmsColumn_photons)) deallocate (atmsColumn_photons)
  if (allocated(vertical_weights)) deallocate (vertical_weights)  
  !
...
! -------------------------------------------------------------------------------
  subroutine writeResults_netcdf(domainFileName,  numPhotonsPerBatch, numBatches, &
    ...
    if(recScatOrd) then
      ...
      ncStatus( 2) = nf90_put_var(ncFileId, ncVarId, real((/0, 1:numRecScatOrd/)))
